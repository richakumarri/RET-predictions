...
SET start_date =  dateadd('month', -6, date_trunc('month',  current_date())) ;--since has 180 days feature
...
SET current_month =DATE_TRUNC('MONTH', CURRENT_DATE) ;--2024-07-01
 ...
 create  table scratch.riders.prediction_query_WOD_hour_day_level as
with base as
(
select
   ID
...
select * from summary where  order_month =$current_month;
